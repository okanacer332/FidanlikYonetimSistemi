...
import {
  Alert, Autocomplete, Box, Button, CircularProgress, Divider, Grid, IconButton, Paper, Stack,
  Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography
} from '@mui/material';
import { Plus as PlusIcon, Trash as TrashIcon } from '@phosphor-icons/react';

import type { Plant, Warehouse, Customer, Order } from '@/types/nursery'; // Order tipini import et
...
interface OrderCreateFormProps {
  onSuccess: (newOrder: Order) => void; // DÜZELTME: onSuccess artık yeni siparişi parametre alacak
  onCancel: () => void;
}
...
export function OrderCreateForm({ onSuccess, onCancel }: OrderCreateFormProps): React.JSX.Element {
  ...
  const onSubmit = React.useCallback(async (values: FormValues): Promise<void> => {
    ...
    try {
      ...
      const newOrder = await response.json(); // DÜZELTME: API'den dönen veriyi al

      if (!response.ok) {
        throw new Error(newOrder.message || 'Sipariş oluşturulamadı.');
      }
      onSuccess(newOrder); // DÜZELTME: Yeni sipariş verisini onSuccess ile ana sayfaya gönder
    } catch (err) {
      setFormError(err instanceof Error ? err.message : 'Bilinmeyen bir hata oluştu.');
    }
  }, [onSuccess]);
  ...
  return (
      <form onSubmit={handleSubmit(onSubmit)}>
          {/* ... Formun geri kalan JSX kısmı aynı kalacak ... */}
